...
public class Solution_LRCR {
	...
	static public int characterReplacement(String s, int k) {
		...
		for(right = 0; right < s.length(); right++) {
			...
			int windowLen = right - left + 1; // get size of window
			while( windowLen - Collections.max(cMap.values()) > k) {
				key = s.charAt(left);          //if window invalid, remove one character
				cnt = cMap.get(key);           //from left until window valid
				cMap.put(key, cnt - 1);        //Decrement character count
				left++;
